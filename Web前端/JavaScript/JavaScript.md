...
**MDN的JavaScript教程**：https://developer.mozilla.org/zh-CN/docs/Web/JavaScript
...
JavaScrip的核心语法部分相当精简，只包括两部分：基本语法
...
![image-20201115202221685](JavaScript基础.assets/image-20201115202221685.png)

**保留字**

![image-20201115202241968](JavaScript基础.assets/image-20201115202241968.png)
...
| document.write   | 在页面输出内容（==仅用于测试==）                             |
| innerHTML        | 写入HTML元素                                                 |

**使用innerHTML**

可以使用document.getElementById(id)方法访问HTML元素。
...
JavaScript把数据类型分为两类：==简单（基本）数据类型==，==复杂（引用）数据类型==。

> * 简单（基本）数据类型：Number（数值），String（字符串），Boolean（布尔），Undefined（未定义），Null（对空），Symbol（ES6引入）。
> * 复杂（引用）数据类型：Object（对象），Array（数组），函数（function）。
...
> * 如果数值运算超过了数值类型表示的最大值，那么结果将是==Infinity（正无穷）==。
...
对象是一组数据和功能的集合。Object类型所具有的任何属性和方法也同样存在于更具体的对象中。

Object的每个实例都具有下列属性和方法：

> * constructor：保存着用于创建当前对象的函数。
> * hasOwnProperty(propertyName)：用于检查给定的属性在当前对象实例中（而不是在实例原型中）是否存在。其中，作为参数的属性名(propertyName)必须以字符串形式指定。例：o.hasProperty("name")。
> * isProtoTypeOf(object)：用于检查传入的对象是否是另一个对象的原型。
> * propertyIsEnumerable(propertyName)：用于检查给定的属性是否能够使用for-in语句。与hasOwnProperty()方法一样，作为参数的属性名必须以字符串的形式指定。
...
当不确定有多少个参数传递时，可以用arguments来获取。arguments对象中存储了传递进来的所有实参。

### 函数表达式（匿名函数）

用于声明函数

函数表达式的声明方式和变量的声明方式差不多，只不过变量里存放的是值，而函数表达式里存放的是变量。
